...
namespace Docear4Word
{
	...
	public class AddinModule: ADXAddinModule
	{
		...
		const string AddReferenceCaption = "Add Reference";
		const string EditReferenceCaption = "Edit Reference";
		private ADXRibbonBox adxRibbonBox4;

		MainController mainController;
		private ADXRibbonButton rbSettings;
		private ADXCommandBarButton cbbSettings;
...
// ReSharper restore RedundantOverridenMember
		...
		public void UpdateState(SelectionManager selectionManager)
		{
			...
			var fieldMatchCount = hasSelectionManager ? selectionManager.FieldMatches.Count : 0;

			var canAdd = fullyEnabled && fieldMatchCount == 0;
			var canEdit = fullyEnabled && selectionManager.IsSingleCitation;
			var canInsertBibliography = fullyEnabled && fieldMatchCount == 0;

			rbAddReference.Visible = !canEdit;
			rbAddReference.Enabled = canAdd;
			rbEditReference.Visible = canEdit;
			rbRefresh.Enabled = fullyEnabled;
			rbInsertBibliography.Enabled = canInsertBibliography;
			rcbStyle.Enabled = partlyEnabled;
			rbMovePrevious.Enabled = partlyEnabled;
			rbMoveNext.Enabled = partlyEnabled;

			rbInsertTestData.Visible = showInsertTestDataButton;
			rbInsertTestData.Enabled = fullyEnabled;

			cbbAddReference.Visible = !canEdit;
			cbbAddReference.Enabled = canAdd;
			cbbEditReference.Visible = /*cbbEditReference.Enabled = */canEdit;
			cbbRefresh.Enabled = fullyEnabled;
			cbbInsertBibliography.Enabled = canInsertBibliography;
			ccbStyle.Enabled = partlyEnabled;
			cbbMovePrevious.Enabled = partlyEnabled;
			cbbMoveNext.Enabled = partlyEnabled;
		}
        ...
        public static IAccessible GetAccessibleObjectFromHandle(IntPtr hwnd)
        {
            ...
            if (hwnd != (IntPtr)0)
            {
                AccessibleObjectFromWindow(hwnd, 0, ref guidAccessible, ref accObject);
